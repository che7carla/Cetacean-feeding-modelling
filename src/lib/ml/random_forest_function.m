%% Function to train and test a random forest classification model
%% Input:
% 1) trainingDataset: 
...
random_forest_settings_optimized = fitcensemble( ...
    predictors, ... 
    response, ...
    'Method', 'Bag', ...
...
trainedModel.ClassificationEnsemble = random_forest_settings_optimized;
...
C_validation_RF=confusionmat(table2array(trainingDataset(:, targetFeatureName)), validationPredictions);
TP_val_RF = C_validation_RF(1, 1);
TN_val_RF = C_validation_RF(2, 2);
FP_val_RF = C_validation_RF(2, 1);
FN_val_RF = C_validation_RF(1, 2);
validationAccuracy_RF = (TP_val_RF+TN_val_RF)/(TP_val_RF+TN_val_RF+FN_val_RF+FP_val_RF);
validationSensitivity_RF = TP_val_RF / (FN_val_RF + TP_val_RF);
validationSpecificity_RF = TN_val_RF / (TN_val_RF + FP_val_RF);
validationBCR_RF= (validationSensitivity_RF + validationSpecificity_RF)/2;
...
writetable(featuresImportanceTable, "Features_importance_RF.xlsx", 'WriteRowNames',true);
...
validation_results.validation_accuracy = validationAccuracy_RF;
validation_results.validation_specificity = validationSpecificity_RF;
validation_results.validation_sensitivity= validationSensitivity_RF;
validation_results.validation_BCR= validationBCR_RF;
